...
12. [CDP-режим — собственный стейблкоин ceitUSD](#12-cdp-режим--собственный-стейблкоин-ceitusd)
13. [PSM — обмен ceitUSD ↔ USDC](#13-psm--обмен-ceitusd--usdc)
...
│   ├── CeitnotUSD.sol               # Mintable-стейблкоин (ceitUSD) для CDP-режима
...
## 12. CDP-режим — собственный стейблкоин ceitUSD

### Что такое CDP

**CDP (Collateralized Debt Position)** — это позиция, в которой вы вносите залог и получаете **собственный стейблкоин протокола** — **ceitUSD**.

В обычном режиме (раздел 9) Ceitnot выдаёт займ в **внешнем** стейблкоине (USDC). Но в CDP-режиме протокол **минтит собственный** стейблкоин — ceitUSD. Это как MakerDAO → DAI или Liquity → LUSD.

### Зачем свой стейблкоин?

1. **Не зависим от USDC** — не нужно держать ликвидность USDC в пуле
2. **Масштабируемость** — можно выдавать столько ceitUSD, сколько позволяет коллатерал
3. **Revenue** — комиссии за минт/берн ceitUSD идут в Treasury → держателям VeCEITNOT (после распределения через `VeCeitnot`)
4. **PSM** — через Peg Stability Module (раздел 13) пользователь может обменять ceitUSD ↔ USDC

### Как работает

```
Пользователь → depositCollateral(wstETH) → borrow(ceitUSD)
                                              ↓
                                         ceitUSD минтится из контракта CeitnotUSD.sol
                                              ↓
                                    Пользователь имеет ceitUSD на кошельке
```

1. Пользователь вносит коллатерал (shares ERC-4626 vault)
2. Вызывает `borrow()` — движок минтит ceitUSD через `CeitnotUSD.mint()`
3. ceitUSD приходит на кошелёк пользователя
4. При погашении: пользователь вызывает `repay()` — ceitUSD сжигается (`burn()`)

### Стейблкоин ceitUSD — контракт `CeitnotUSD`
...
- Залог остаётся в движке, ceitUSD — чистый долговой токен
...
3. Borrow → получить ceitUSD (минтится)
4. PSM: обменять ceitUSD → USDC (реальные доллары!)
5. Пользователь использует USDC как хочет
6. Когда готов вернуть: USDC → ceitUSD через PSM
7. Repay ceitUSD → ceitUSD сжигается, долг закрыт
8. Withdraw коллатерал
```

---

## 13. PSM — обмен ceitUSD ↔ USDC

### Что такое PSM

**PSM (Peg Stability Module)** — контракт, который позволяет обменивать ceitUSD ↔ USDC по курсу **1:1** (с небольшой комиссией).

Это нужно, чтобы ceitUSD всегда стоил ~$1. Если ceitUSD стоит $0.99 — арбитражники купят ceitUSD дёшево на рынке и обменяют на USDC через PSM (прибыль). Это вернёт цену к $1. И наоборот.
...
Основные функции:
- `swapCeitUsdForUsdc(amount)` — отдаёшь ceitUSD, получаешь USDC
- `swapUsdcForCeitUsd(amount)` — отдаёшь USDC, получаешь ceitUSD
- Комиссия: настраивается (по умолчанию 0.1% = 10 bps)

### Как использовать (на сайте)

1. Получите ceitUSD через borrow (раздел 12)
2. На сайте: вкладка **PSM** → введите сумму ceitUSD → **Swap to USDC**
3. ceitUSD сжигается, USDC приходит на кошелёк
4. Обратно: введите USDC → **Swap to ceitUSD** → ceitUSD минтится

### Как использовать (в терминале)

```powershell
# Approve ceitUSD для PSM
...
Начальная ликвидность — от команды/инвесторов. Потом USDC накапливаются от пользователей, которые делают swap USDC → ceitUSD.
...
Скрипт деплоит полный стек Ceitnot: Engine, Registry, Oracle, Vault, ceitUSD, PSM, Router, Treasury, `CeitnotToken`, `VeCeitnot`, `CeitnotGovernor`, Timelock.
