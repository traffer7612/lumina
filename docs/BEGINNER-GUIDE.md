...
14. [Governance — veAURA и голосование](#14-governance--veaura-и-голосование)
...
В обычном режиме (раздел 9) Aura выдаёт займ в **внешнем** стейблкоине (USDC). Но в CDP-режиме протокол **минтит собственный** стейблкоин — aUSD. Это как MakerDAO → DAI или Liquity → LUSD.
...
## 14. Governance — veAURA и голосование

### Зачем это нужно

Aura Protocol — это как банк, которым управляют сами пользователи. Чтобы голосовать за решения протокола, нужно доказать свою заинтересованность — заморозить свои AURA токены.
...
- **VeAura** (`src/governance/VeAura.sol`) — лок AURA, подсчёт voting power, распределение revenue
...
На сайте Aura есть вкладка **Governance** (`/governance`), где можно:
...
5. Запустить фронтенд на кастомном домене (например aura.finance)
