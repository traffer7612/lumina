...
- 🟡 Define maximum supply policy (hard cap or governed inflation path). (mechanics in [`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md); **publish numbers**)
- 🟡 Document initial circulating supply and fully diluted supply assumptions. (same — **fill at launch**)
- 🟢 Document all mint roles and mint permissions (who can mint, through which contract path). ([`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md) §2)
...
- 🟡 Confirm no hidden mint path exists outside documented governance flow. (verification steps in [`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md) §2 — **run before mainnet comms**)
...
- 🟡 Publish governance anti-capture policy (turnout expectations, emergency response). ([`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md) §8 — **add targets**)
- 🟡 Verify thresholds are realistic for current ve supply and participation. (on-chain read + community review)

## 3) Revenue & Fee Flow

- 🟢 Publish protocol fee sources (CDP, PSM, liquidation, other). ([`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md) §5)
- 🟡 Publish destination of each fee stream (treasury, ve rewards, reserves, buyback). ([§6](CEITNOT-TOKENOMICS-AND-FEES.md) — **confirm %/routing**)
- 🟡 Publish accounting cadence (how often distribution/claim updates happen). ([§6](CEITNOT-TOKENOMICS-AND-FEES.md) — **state schedule**)
- 🟡 Publish treasury wallet policy and spending authorization path. ([§7](CEITNOT-TOKENOMICS-AND-FEES.md) — **fill addresses/rules**)
- 🟡 Add monitoring for fee inflow/outflow addresses. ([§10](CEITNOT-TOKENOMICS-AND-FEES.md) — **configure alerts**)
...
- 🟡 Publish treasury composition (stable, volatile, LP, protocol-owned assets). ([`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md) §9 template)
...
- 🟡 Confirm on-chain alerts (role changes, minter changes, pauses, large outflows). ([`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md) §10 — **enable tooling**)
...
- Role/minter change tx hashes: *e.g. `CeitnotUSD.addMinter(new PSM)` after queue; later `removeMinter(legacy PSM)`*
- Final market parameter proposal IDs:
- Public docs PR links: *this checklist + [`PRODUCTION-ADDRESSES-ARBITRUM.md`](PRODUCTION-ADDRESSES-ARBITRUM.md), [`CEITNOT-TOKENOMICS-AND-FEES.md`](CEITNOT-TOKENOMICS-AND-FEES.md), [`PSM-ORACLE-MARKET-RISK.md`](PSM-ORACLE-MARKET-RISK.md), [`INCIDENT-ROLLBACK-RUNBOOK.md`](INCIDENT-ROLLBACK-RUNBOOK.md)*
