...
Краткое описание для новичка: **Ceitnot** — это протокол займов под залог (lending). Пользователь вносит коллатерал (например wstETH), получает право занимать стейблкоин (в CDP-режиме — **ceitUSD**). Цена коллатерала берётся из оракула; логику движка можно обновлять через прокси. Имена контрактов в коде — **`CeitnotEngine`**, **`CeitnotProxy`** и т.д.; см. [BRANDING-AND-NAMING.md](BRANDING-AND-NAMING.md).
...
└──────────────────────┘ └─────────────────┘ └─────────────────┘
         ...
         ▼
┌─────────────────┐
│  Debt token     │  ← Например USDC (legacy) или ceitUSD (CDP)
└─────────────────┘
```
...
- **`CeitnotEngine`**  
  Ядро протокола Ceitnot: депозит/вывод коллатерала по `marketId`, займ и погашение (в CDP — ceitUSD), учёт долга и коллатерала, ликвидации, harvest, пауза и emergency shutdown. Состояние в `CeitnotStorage` (EIP-7201), реестр рынков и оракул.
...
6. `CeitnotProxy` перенаправляет вызов в `CeitnotEngine`. Движок проверяет LTV, реестр рынков и оракул; в legacy-режиме со стейблом на балансе движка — списывает долг-токен и переводит пользователю (в CDP — минт/берн ceitUSD).
