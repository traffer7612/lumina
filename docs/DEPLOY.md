...
Если нужен **тот же стек, что на Sepolia** (не legacy USDC в движке): **ceitUSD** как долг, **CDP** (`setMintableDebtToken(true)`), **PSM** (ceitUSD ↔ USDC), **Router**, **Treasury**, контракты **`CeitnotToken` + `VeCeitnot` + `CeitnotGovernor` + Timelock** — используйте скрипт `script/DeployFullProduction.s.sol`.
...
- После CDP-деплоя **не нужно** заливать USDC на движок для выдачи займов: borrow **минтит ceitUSD**. USDC на движке нужен только в сценарии **legacy** (`DeployProduction`).
- В `CeitnotPSM` комментарий в коде про 1:1 в **сырых** единицах: у **нативного USDC 6 decimals**, у **ceitUSD — 18**. Перед продакшеном проверьте UX/математику свопов; CDP borrow/repay не зависят от PSM.
...
**Вариант B — CDP + ceitUSD + PSM + governance (как на Sepolia):**
