...
- задеплоит движок Lumina (`AuraEngine`) и прокси (`AuraProxy`);
...
Если нужен **тот же стек, что на Sepolia** (не legacy USDC в движке): **aUSD** как долг, **CDP** (`setMintableDebtToken(true)`), **PSM** (aUSD ↔ USDC), **Router**, **Treasury**, контракты **`AuraToken` + `VeAura` + `AuraGovernor` + Timelock** — используйте скрипт `script/DeployFullProduction.s.sol`.
...
|------------|---------------------|
| `VITE_ENGINE_ADDRESS` | Адрес прокси движка Lumina (`AuraProxy` → `AuraEngine`) на Sepolia. Проверьте строку целиком: в середине должно быть **`…F353C…`**, не `…F33C…` (иначе адрес неверный и маркеты не загрузятся). |
| `VITE_REGISTRY_ADDRESS` | Реестр маркетов |
| `VITE_AURA_TOKEN_ADDRESS` | Токен LUMINA в UI (контракт `AuraToken`; в кошельке может быть тикер AURA) |
| `VITE_VE_AURA_ADDRESS` | veLUMINA в UI (контракт `VeAura`) |
